...
# Cloudinary — Media Storage (production: Cloudinary, development: local)
CLOUDINARY_STORAGE = {
...
# production uses certifi CA bundle). For production, switch to SendGrid by either:
#   1. Setting EMAIL_BACKEND=sendgrid_backend.SendgridBackend and SENDGRID_API_KEY=...
#   2. Or setting EMAIL_HOST_USER/EMAIL_HOST_PASSWORD for standard SMTP
#
# Default: certifi-backed SMTP (works for both dev & prod SMTP setups)
EMAIL_BACKEND = config(
    'EMAIL_BACKEND',
    default='apps.users.email_backend.CertifiEmailBackend',
)
...
SENDGRID_SANDBOX_MODE_IN_DEBUG = config('SENDGRID_SANDBOX_MODE', default=False, cast=bool)

# Note: EMAIL_SSL_CERTFILE is for client certificate auth, not CA bundles.
# SSL verification is handled by the CertifiEmailBackend (apps/users/email_backend.py).
